...
const MONTHS_DARI = ['حمل', 'ثور', 'جوزا', 'سرطان', 'اسد', 'سنبله', 'میزان', 'عقرب', 'قوس', 'جدی', 'دلو', 'حوت'];
const MONTHS_PASHTO = ['وری', 'غویی', 'غبرګولی', 'چنګاښ', 'زمری', 'وږی', 'تله', 'لړم', 'لړم', 'مرغومی', 'سلواغه', 'کب'];

// Note: the Pashto list above uses commonly seen names; you can adjust order/names as desired.
const SKIP_TAGS = new Set(['SCRIPT', 'STYLE', 'TEXTAREA', 'INPUT', 'CODE', 'PRE', 'NOSCRIPT', 'IFRAME']);
...
function toAsciiDigits(str) {
  return str.replace(/[۰-۹]/g, ch => String(PERSIAN_DIGITS.indexOf(ch)))
...
function norm(str) { return str.replace(/\u200e|\u200f/g, '').trim(); }
...
function jalaliToGregorian(jy, jm, jd) {
  ...
  var gd = days + 1;
  var sal_a = [0, 31, ((isGregorianLeap(gy)) ? 29 : 28), 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
  var gm = 0;
  for (var j = 1; j < 13; j++) {
    var v = sal_a[j];
    if (gd <= v) { gm = j; break; }
...
const EN_MONTHS_MAP = (function () {
  ...
  for (let i = 0; i < names.length; i++) { m[names[i]] = i + 1; m[names[i].slice(0, 3)] = i + 1; }
  return m;
})();
// Persian (Jalali) numeric pattern like 1403/08/18 (possibly Persian digits)
const JALALI_RE = /\b[۰-۹0-9]{3,4}[\/\-\.\s][۰-۹0-9]{1,2}[\/\-\.\s][۰-۹0-9]{1,2}\b/;
// Hijri pattern like 1447/05/12 (possibly Arabic-Indic)
const HIJRI_RE = /\b[٠-٩0-9]{3,4}[\/\-\.\s][٠-٩0-9]{1,2}[\/\-\.\s][٠-٩0-9]{1,2}\b/;

// Attempt parse any date-like substring and return JS Date or null
// Only returns dates with Gregorian year > 2000
function tryParseDateFromString(text) {
  ...
  if (m) {
    ...
    // Heuristics: if year >= 2000 and <= 9999 -> treat as Gregorian year first
    if (a >= 2000 && a <= 9999) {
      // Gregorian attempt
      const g = new Date(a, b - 1, c);
      if (!isNaN(g) && g.getFullYear() > 2000) return g;
    }
    ...
    if (a >= 1300 && a <= 1500) {
      ...
      // Only return if Gregorian year > 2000
      if (!isNaN(gDate) && gDate.getFullYear() > 2000) return gDate;
    }
  }
  ...
  if (m) {
    ...
    if (y > 2000) {
      ...
      if (!isNaN(gg) && gg.getFullYear() > 2000) return gg;
  ...
  const enMonthRegex = new RegExp('\\b(' + Object.keys(EN_MONTHS_MAP).join('|') + ')\\b', 'i');
  if (enMonthRegex.test(s)) {
    // try patterns
    // a) MonthName DD, YYYY
    let re1 = /([A-Za-z]{3,9})[,\s\-]+([0-9]{1,2})(?:st|nd|rd|th)?,?\s*([0-9]{3,4})?/i;
    let r1 = s.match(re1);
    if (r1) {
  ...
  if (m) {
    ...
    const parts = en.split(/[\/\-\.\s]+/).map(Number);
    if (parts.length >= 3) {
      const hy = parts[0], hm = parts[1], hd = parts[2];
      if (hy >= 1400 && hy <= 1500) { // common Hijri years approx range
  ...
  let simpleJ = s.match(/([۰-۹0-9]{3,4})\s+(فروردین|اردیبهشت|خرداد|تیر|مرداد|شهریور|مهر|آبان|آذر|دی|بهمن|اسفند|حمل|ثور|جوزا|سرطان|اسد|سنبله|میزان|عقرب|قوس|جدی|دلو|حوت)/i);
  if (simpleJ) {
    ...
    const persianMap = {
      'حمل': 1, 'ثور': 2, 'جوزا': 3, 'سرطان': 4, 'اسد': 5, 'سنبله': 6, 'میزان': 7, 'عقرب': 8, 'قوس': 9, 'جدی': 10, 'دلو': 11, 'حوت': 12,
...
function walkAndReplace(root, locale, opts) {
  ...
  while ((node = walker.nextNode())) {
    ...
    while ((m = candidateRE.exec(text)) !== null) {
      ...
      if (parsed) {
        const formatted = formatSolarHijriForLocale(parsed, locale, opts.usePersianDigits);
        // replace the specific substring occurrence (only the matched substring)
        const start = idx, end = idx + substr.length;
        newText = newText.slice(0, start + offsetShift) + formatted + newText.slice(end + offsetShift);
...
function updateTimeElements(locale, opts) {
  ...
  document.querySelectorAll('time[datetime]').forEach(t => {
    try {
      ...
      // Only convert if year > 2000
      if (!isNaN(d) && d.getFullYear() > 2000) {
        t.textContent = formatSolarHijriForLocale(d, locale, opts.usePersianDigits);
      }
    } catch (e) { }
  });
}
